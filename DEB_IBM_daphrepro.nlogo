...
to go 
 ...
 ask turtles 
  [ 
    if U_H >= U_H^p 
      [
        set repro-time repro-time + (1 / timestep) 
        if repro-time > days-between-repro  
        [
          calc-lay-eggs
          if lay-egg? = 1
          [
...
to calc-lay-eggs
  ...
  loop [   
    ...
    if e_scaled_embryo < e_scaled [set repro-time 0 stop]
    ] 
end 
...
; individuals update their state variables based on the calc_state variable proccesses
  ...
  [
   ...
   if aging = "on" [if ticks mod timestep = age-day [if random-float 1 < h_rate [set die? 1]] ] ;ageing related mortality
   if aging = "off" [if ticks mod timestep = age-day [if random-float 1 < background-mortality [set die? 1]] ]
   
   if die? = 1 and U_H >= U_H^p 
      [ 
        let m-id id
        let offspring turtles with [mother-id = m-id and U_H < U_H^b]
        if any? offspring [ask offspring [die] ]
        die ; the mother then dies 
      ]
    if die? = 1 [die] 
